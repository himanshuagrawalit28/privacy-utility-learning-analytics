...
export default function Analytics() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card rounded-2xl p-6 border border-cyan-500/30 shadow-md">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
          ...
          <div className="flex items-center gap-3">
            <div className="px-3.5 py-1.5 rounded-xl bg-white border border-slate-200 text-xs font-mono">
              <span className="text-slate-500">Target: </span>
              <span className="text-blue-700 font-bold">ε = {selectedEpsilon}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs text-slate-500 font-mono">
            ...
            <span className="text-blue-600 font-bold">Recommended Zone (1.0 - 2.0)</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-5 pt-5 border-t border-slate-200">
          <div className="p-3.5 rounded-xl bg-slate-50/60 border border-slate-200 text-center">
            <div className="text-xs text-slate-500">Projected Model Accuracy</div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50/60 border border-slate-200 text-center">
            <div className="text-xs text-slate-500">Projected Privacy Score</div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50/60 border border-slate-200 text-center">
            <div className="text-xs text-slate-500">MIA Defense Efficacy</div>
      ...
      <div className="glass-card rounded-2xl p-6 border border-slate-200 shadow-md">
      ...
      <div className="glass-card rounded-2xl p-6 border border-slate-200">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-white/90 text-slate-500 uppercase tracking-wider font-mono border-b border-slate-200">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {tradeoffData.map((row) => {
                ...
                return (
                  <tr key={row.epsilon} className={`hover:bg-slate-100/30 ${isSelected ? 'bg-blue-100' : ''}`}>
                    <td className="py-3 px-4 font-semibold text-slate-800">
                      ε = {row.epsilon}
                      {isSelected && <span className="ml-2 text-[10px] text-blue-600 font-sans font-bold">(ACTIVE)</span>}
                    </td>
                    <td className="py-3 px-4 text-center text-indigo-300 font-bold">{row.accuracy}%</td>
                    <td className="py-3 px-4 text-center text-blue-700 font-bold">{row.privacyScore}%</td>
                    <td className="py-3 px-4 text-center text-emerald-400 font-semibold">{row.attackSuccess}%</td>
                    <td className="py-3 px-4 text-right">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] ${
                        isSelected ? 'bg-cyan-500/20 text-blue-700 border border-cyan-500/40' :
